...
def _explicit_conflict_reasons(
    ...
    generic = "conflicting-explicit-provider-identities"
    if identities and all(identity.provider == "tvmaze" for identity in identities):
        return ("conflicting-explicit-tvmaze-ids", generic)
    return (generic,)
...
def resolve_show_group_with_provider(
    ...
    """Resolve one source-show group through a normalized metadata provider."""
