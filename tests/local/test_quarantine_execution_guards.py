...
def _organize_winner(source: Path, organized: Path) -> None:
    src = source / "Release" / "winner.mkv"
    dst = organized / "Show (2026)" / "Season 01" / "winner.mkv"
...
def test_root_and_journal_location_guards_fail_closed(tmp_path: Path) -> None:
    source, organized, quarantine, prepared = _case(tmp_path)
    missing = tmp_path / "missing"
    with pytest.raises(QuarantineExecutionError, match="root does not exist"):
        execute_quarantine(
            prepared, source, organized, missing, journal_path=None, check_only=True
        )

    not_directory = tmp_path / "file-root"
    not_directory.write_text("x", encoding="utf-8")
    with pytest.raises(QuarantineExecutionError, match="must be a directory"):
        quarantine_execution._validated_root(not_directory, "test")

    nested_source = source / "nested-quarantine"
    nested_source.mkdir()
    ...
    nested_organized = organized / "nested-quarantine"
...
def test_candidate_parent_and_mapping_guards(tmp_path: Path) -> None:
    source, _organized, _quarantine, _prepared = _case(tmp_path)
    with pytest.raises(QuarantineExecutionError, match="required member parent is missing"):
...
def test_winner_ambiguous_live_states_are_rejected(tmp_path: Path) -> None:
    source, organized, quarantine, prepared = _case(tmp_path / "both")
    winner = prepared.plan.groups[0].winner
    ...
    with pytest.raises(
        QuarantineExecutionError, match="organized destination fingerprint is wrong"
    ):
        execute_quarantine(
            prepared, source, organized, quarantine, journal_path=None, check_only=True
        )

    assert winner.source_relative_path == "Release/winner.mkv"


def test_loser_ambiguous_live_states_are_rejected(tmp_path: Path) -> None:
    ...
    (source / "Release" / "loser.mkv").unlink()
    target = quarantine / "Release" / "loser.mkv"
    target.parent.mkdir()
...
def test_quarantine_control_flow_guards_and_completed_reentry(tmp_path: Path) -> None:
    source, organized, quarantine, prepared = _case(tmp_path)
    ...
    with pytest.raises(QuarantineExecutionError, match="explicit journal"):
        execute_quarantine(
            prepared, source, organized, quarantine, journal_path=None
        )
    ...
    assert again.members_recovered == 0

    other = tmp_path / "existing.jsonl"
    other.write_text("occupied", encoding="utf-8")
    source2, organized2, quarantine2, prepared2 = _case(tmp_path / "existing-case")
    ...
    missing_journal = tmp_path / "missing-resume.jsonl"
...
def test_invalid_and_empty_quarantine_journals_are_rejected(tmp_path: Path) -> None:
...
def test_restore_guards_tamper_and_completed_reentry(tmp_path: Path) -> None:
    ...
    with pytest.raises(QuarantineExecutionError, match="changed after restore approval"):
        execute_quarantine_restore(
            ...
            organized,
            quarantine,
            restore_journal_path=tmp_path / "after-tamper.jsonl",
        )


def test_prepare_restore_and_restore_journal_guards(tmp_path: Path) -> None:
    source, organized, quarantine, prepared = _case(tmp_path)
    del source, organized, quarantine
    ...
    journal = quarantine_execution._QuarantineJournal(incomplete, prepared, resume=False)
...
def test_confirmation_tokens_bind_roots_and_quarantine_journal(tmp_path: Path) -> None:
    source, organized, quarantine, prepared = _case(tmp_path / "token")
    token = quarantine_token(prepared, source, organized, quarantine)
    assert token.startswith("QUARANTINE:")

    other_quarantine = tmp_path / "token" / "other-quarantine"
    other_quarantine.mkdir()
    assert token != quarantine_token(prepared, source, organized, other_quarantine)

    _organize_winner(source, organized)
    journal = tmp_path / "token" / "quarantine.jsonl"
    execute_quarantine(
        prepared, source, organized, quarantine, journal_path=journal
    )
    restore = prepare_quarantine_restore(prepared, journal)
...
def test_journal_state_tracks_started_completed_and_rollback() -> None:
    entries = [
        {"event": "member-started", "group_id": "g", "source_relative_path": "A.mkv"},
        {"event": "member-completed", "group_id": "g", "source_relative_path": "A.mkv"},
        {
            "event": "member-rollback-completed",
            "group_id": "g",
            "source_relative_path": "A.mkv",
        },
        {"event": "group-completed", "group_id": "g", "source_relative_path": None},
        {"event": "run-completed", "group_id": None, "source_relative_path": None},
    ]
    state = quarantine_execution._journal_state(entries, completion_event="run-completed")
...
def test_sha_read_failure_is_wrapped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(QuarantineExecutionError, match="is unreadable"):
        quarantine_execution._sha256_file(path, "journal")
